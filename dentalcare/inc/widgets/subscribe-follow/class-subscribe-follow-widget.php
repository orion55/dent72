...
class Dentalcare_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'dentalcare' );
		$this->widget_id          = 'dentalcare_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'dentalcare' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'dentalcare' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'dentalcare' ),
				'label' => esc_html__( 'Subscribe Title', 'dentalcare' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'dentalcare' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'dentalcare' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'dentalcare' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'dentalcare' ),
				'label' => esc_html__( 'Subscribe submit label', 'dentalcare' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'dentalcare' ),
				'label' => esc_html__( 'Subscribe success', 'dentalcare' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'dentalcare' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'dentalcare' ),
				'label' => esc_html__( 'Follow Title', 'dentalcare' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'dentalcare' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'dentalcare' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'dentalcare' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'dentalcare' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'dentalcare' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'dentalcare' ),
				'value'   => 'center',
				'options' => array(
			...
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'dentalcare' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'dentalcare' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'dentalcare' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'dentalcare' ),
				'value'   => 'inherit',
				'options' => array(
			...
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'dentalcare' ),
				'value'  => '#000000',
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_dentalcare_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_dentalcare_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'dentalcare' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'dentalcare' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'dentalcare' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'dentalcare' ),
		);

		$this->nonce = wp_create_nonce( 'dentalcare-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		$follow_template            = dentalcare_get_locate_template( 'inc/widgets/subscribe-follow/view/follow-view.php' );
		$subscribe_template         = dentalcare_get_locate_template( 'inc/widgets/subscribe-follow/view/subcribe-view.php' );
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'dentalcare' );
		}
	...
	public function get_social_nav() {
		return dentalcare_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = dentalcare_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'dentalcare-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'dentalcare_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function dentalcare_register_subscribe_follow_widgets() {
	register_widget( 'Dentalcare_Subscribe_Follow_Widget' );
}
