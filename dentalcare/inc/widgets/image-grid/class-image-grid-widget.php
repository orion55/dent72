 ...
 * @package Dentalcare
 */
if ( ! class_exists( 'Dentalcare_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class Dentalcare_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Image Grid', 'dentalcare' );
			$this->widget_description = esc_html__( 'This widget displays images from post.', 'dentalcare' );
			...
			$this->settings           = array(
				...
				'category_name' => array(
					...
					'label'            => esc_html__( 'Select categories to show', 'dentalcare' ),
				...
				'post_sort' => array(
					...
					'value'   => 'date',
					'options' => array(
						'date'          => esc_html__( 'Publish Date', 'dentalcare' ),
						'title'         => esc_html__( 'Post Title', 'dentalcare' ),
						'comment_count' => esc_html__( 'Comment Count', 'dentalcare' ),
					),
					'label'   => esc_html__( 'Post sorted', 'dentalcare' ),
				),
				'post_number' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Posts number', 'dentalcare' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number', 'dentalcare' ),
				),
				'items_padding' => array(
					'type'       => 'stepper',
					'value'      => '5',
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'dentalcare_image_grid_widget_args', $args, $instance );

			if ( $this->get_cached_widget( $args ) ) {
				return;
			}

			$template = dentalcare_get_locate_template( '/inc/widgets/image-grid/views/image-grid-view.php' );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'       => apply_filters( 'dentalcare_order_image_grid_widget', 'DESC', $instance ), // ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				$grid_class_array = apply_filters( 'dentalcare_image_grid_widget_grid_class', array(
				...
				echo apply_filters(
					'dentalcare_image_grid_widget_before',
				...
				foreach ( $posts as $post ) {
					...
					$image = $this->utility->media->get_image( array(
						'size'              => 'dentalcare-thumb-m',
						'mobile_size'       => 'dentalcare-thumb-m',
				...
				echo apply_filters( 'dentalcare_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'dentalcare_register_image_grid_widget' );
	/**
	 * Register image grid widget.
	 */
	function dentalcare_register_image_grid_widget() {
		register_widget( 'Dentalcare_Image_Grid_Widget' );
	}
}
