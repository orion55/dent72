...
if ( ! class_exists( 'Dentalcare_Custom_Posts_Widget' ) ) {

	/**
	 * Class Dentalcare_Custom_Posts_Widget.
	 */
	class Dentalcare_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'dentalcare' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'dentalcare' );
			$this->widget_id          = apply_filters( 'dentalcare_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'dentalcare_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = dentalcare_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'dentalcare' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'dentalcare' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'dentalcare' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'dentalcare' ),
				...
				'post_offset' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'dentalcare' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'dentalcare' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						...
						'comment_count' => esc_html__( 'Comment count', 'dentalcare' ),
						'category'      => esc_html__( 'Category', 'dentalcare' ),
						'post_tag'      => esc_html__( 'Tag', 'dentalcare' ),
						'more_button'   => esc_html__( 'More Button', 'dentalcare' ),
					),
				...
				'button_text' => array(
					...
					'label' => esc_html__( 'Post read more button label', 'dentalcare' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = dentalcare_get_locate_template( 'inc/widgets/custom-posts/views/custom-post-view.php' );
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', dentalcare_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			if ( $posts ) {
				...
				foreach ( $posts as $post ) {
					...
					if (
						...
						$image_size = 'dentalcare-thumb-480-271';
					}
	...
	add_action( 'widgets_init', 'dentalcare_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function dentalcare_register_custom_posts_widget() {
		register_widget( 'Dentalcare_Custom_Posts_Widget' );
	}
}
