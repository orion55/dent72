 ...
 * @package Dentalcare
 */
if ( ! class_exists( 'Dentalcare_Carousel_Widget' ) ) {

	/**
	 * Class Dentalcare_Carousel_Widget.
	 */
	class Dentalcare_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of your posts on your site.', 'dentalcare' );
			$this->widget_id          = 'dentalcare_widget_carousel';
			$this->widget_name        = esc_html__( 'Carousel', 'dentalcare' );
			...
			$this->settings           = array(
				...
				'more_button_text' => array(
					'type'   => 'text',
					'value'  => esc_html__( 'Learn more', 'dentalcare' ),
				...
				'slides_per_view' => array(
					...
					'min_value' => 1,
					'value'     => 4,
					'label'     => esc_html__( 'Number of slides per view', 'dentalcare' ),
				),
				'slides_per_group' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Number slides per group', 'dentalcare' ),
				),
				'slides_per_column' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Multi Row Slides Layout', 'dentalcare' ),
				),
				'space_between_slides' => array(
					...
					'min_value' => 0,
					'value'     => 0,
					'label'     => esc_html__( 'Width of the space between slides(px)', 'dentalcare' ),
				),
				'duration_speed' => array(
					...
					'min_value' => 100,
					'value'     => 500,
					'label'     => esc_html__( 'Duration of transition between slides (ms)', 'dentalcare' ),
				),
				'navigation' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider navigation', 'dentalcare' ),
				),
				'pagination' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider pagination', 'dentalcare' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = dentalcare_get_locate_template( 'inc/widgets/carousel/views/carousel-view.php' );
			...
			// Widgets area check.
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', dentalcare_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="dentalcare-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'dentalcare' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'dentalcare_carousel_default_query_args', array(
				'post_type'      => 'post',
				'orderby'        => 'date',
				'order'          => 'DESC',
				'posts_per_page' => - 1,
		...
		public function get_carousel_loop( $posts_query, $template ) {
			...
			if ( ! $template  ) {
				return '<h5>' . esc_html__( 'View file not found', 'dentalcare' ) . '</h5>';
			}
	...
	add_action( 'widgets_init', 'dentalcare_register_carousel_widget' );
	/**
	 * Register carousel widget.
	 */
	function dentalcare_register_carousel_widget() {
		register_widget( 'Dentalcare_Carousel_Widget' );
	}
}
