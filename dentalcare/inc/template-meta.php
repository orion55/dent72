 ...
 * @return array
 */
function dentalcare_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! dentalcare_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$defaults = apply_filters( 'dentalcare_default_args_share_buttons', array(
		...
		'pinterest' => array(
			...
			'name'      => esc_html__( 'Pinterest', 'dentalcare' ),
	...
	$default_config = apply_filters( 'dentalcare_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'dentalcare_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'dentalcare' ) . $network['name'],
 ...
 * @return string
 */
function dentalcare_get_single_share_buttons() {
	ob_start();
